...
namespace Denix
{
    ...
    public:
        VertexBuffer()
        {
            ...
            if (!m_GL_ID)
                DE_LOG(LogGL, Error, "Failed to create VBO")
        }
                              ...
                              const GLsizei _perPrimitive, const GLuint _count, const GLenum _type)
        {
            ...
            if (BufferSize())
                DE_LOG(LogGL, Trace, "Uploaded data to VBO ID: {}", m_GL_ID)
            else
                DE_LOG(LogGL, Error, "Failed to upload data to VBO ID: {}", m_GL_ID)
        }
    ...
    public:
        IndexBuffer()
        {
            ...
            if (!m_GL_ID)
                DE_LOG(LogGL, Error, "Failed to create IBO")
        }
        ...
        void IndexBufferData(const GLsizei _size, const void* _data, const GLuint _indexCount, const GLenum _type)
        {
            ...
            if (BufferSize())
                DE_LOG(LogGL, Trace, "Uploaded data to IBO ID: {}", m_GL_ID)
            else
                DE_LOG(LogGL, Error, "Failed to upload data to IBO ID: {}", m_GL_ID)
        }
