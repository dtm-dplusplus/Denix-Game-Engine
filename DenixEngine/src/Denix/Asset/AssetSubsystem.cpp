...
namespace Denix
{
    void AssetSubsystem::Initialize()
    {
        ...
        if (Ref<Asset> startSceneAsset = GetSceneAsset(
            FileSubsystem::FormatPath(Engine::GetInstance()->GetConfig().StartupScenePath)))
        {
    ...
    Ref<AudioClip> AssetSubsystem::LoadAudioClip(const Ref<Asset>& _audioClipAsset)
    {
        ...
        {
            DE_LOG(LogAsset, Error, "Load AudioClip: An audio clip name: {} is already loaded",
                   _audioClipAsset->m_AssetName)
    ...
    {
        if (const std::string path = FileSubsystem::FormatRelativePath(_path); s_Instance->m_TextureStore.
            contains(path))
        {
