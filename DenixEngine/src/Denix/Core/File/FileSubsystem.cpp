...
namespace Denix
{
    ...
    {
        return exists(fs::path(FormatPath(_path)).parent_path());
    }
    ...
    bool FileSubsystem::CreateDirectoryDE(const std::string& _path)
    {
        try
        {
            create_directories(fs::path(FormatPath(_path)).parent_path());
            return true;
        }
        catch (const std::exception& e)
        {
    ...
    std::string FileSubsystem::FormatPath(const std::string& _path)
    {
        try
        {
            ...
            return absolute(combinedPath).make_preferred().string();
        }
