...
private:
	static char* FormatName(const char* original) {
		size_t originalLen = strlen(original);
		size_t prefixLen = strlen("class ");
		size_t newLen = originalLen - prefixLen + 1; // +1 for the null terminator

		char* formatted = new char[newLen];
		strcpy(formatted, &original[prefixLen]);
...
template <class T, class BaseClass, ClassID CLID> CRTTI CRTTIClass<T, BaseClass, CLID>::ms_RTTI
	( 0, CLID,  FormatName(typeid(T).name()), BaseClass::GetClassRTTI(), (ClassFactoryFunc)T::Create, 
