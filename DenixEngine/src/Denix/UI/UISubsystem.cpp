...
namespace Denix
{
    void UISubsystem::Initialize()
    {
        ...
        error = FT_New_Face(m_FtLibrary, fontFile.c_str(), 0, &m_Face); /* create face object */
    ...
    void UISubsystem::RenderUISubmission()
    {
        ...
        m_View = lookAt(glm::vec3(0.0f, 0.0f, 15.0f), glm::vec3(0.0f), glm::vec3(0.0f, 1.0f, 0.0f));

        RendererSubsystem::SubmitCamera({m_Projection, m_View});

        for (const auto& widget : m_Widgets)
            for (const auto& child : widget->m_Buttons)
                if (child->m_IsDisplayed) RendererSubsystem::RenderObject({
                    child->m_Material, child->m_Model.lock(), child->m_ModelMatrix
                });
    }
}
